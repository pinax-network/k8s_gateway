...
- `A/AAAA`
- `TXT` when DNS-01 Challenge is enabled
- `CNAME` for services/ingress resources with matching annotation

All other queries result in NODATA responses.
...
```
helm install k8s-gateway oci://ghcr.io/pinax-network/charts/k8s_gateway
```

Alternatively, for labbing and testing purposes `k8s_gateway` can be deployed with a single manifest:

```
kubectl apply -f https://raw.githubusercontent.com/pinax-network/k8s_gateway/master/examples/install-clusterwide.yml
```
...
Alternatively, you can test all the routes using:
